...
- Fundamentals also covers AI *broadly* — an AI overview (what AI is, history, AI/ML/DL hierarchy), full Machine Learning fundamentals (supervised / unsupervised / RL, classical algorithms, evaluation, bias-variance, regularization, feature engineering), full Deep Learning fundamentals (neural networks, backprop, activations, losses, CNNs, RNNs/LSTMs, generative models, embeddings), AI domains (NLP, computer vision, speech, recsys, time series, GNN), and extended math foundations (calculus, statistics, numerical computation) — so a beginner does not have to assume the rest of ML before reaching the LLM chapters, and a deep-learning specialist sees the conceptual map of adjacent subfields.
- The handbook also covers the *responsibility* surface explicitly: a dedicated [Safety](docs/safety/index.md) section (guardrails / alignment / red-teaming / privacy / harm eval), [Security](docs/security/index.md) section (threat models / adversarial attacks / jailbreaks / data poisoning / supply chain / privacy-preserving ML / compliance), [Explainability](docs/explainability/index.md) section (SHAP / LIME / counterfactuals / concept-based / faithfulness), and [Interpretability](docs/interpretability/index.md) section (mechanistic interp, circuits, superposition, sparse autoencoders, interp-for-safety). These are first-class, not afterthoughts.
